select TXCT as TXCTID, TXCTLN.TAX as TAXID, TAX.NME as TAXNME, RATE, sum(SUBT) as SUBT, sum(SUFC) as SUFC, sum(TOTX) as TOTX, sum(TXFC) as TXFC, sum(TOT) as TOT, sum(TOFC) as TOFC
from
